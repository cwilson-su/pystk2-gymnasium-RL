...
def plot_track_3d(csv_file, track_name):
    ...
    output_image = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"{track_name}_track_visualization.png")
    fig.write_image(output_image)  # Save as an image
    print(f"Graph saved as {output_image}")
    
    fig.show()

current_dir = os.path.dirname(os.path.abspath(__file__))  # generating the graph in the script's directory
track_name = "xr591"  # CHANGE ACCORDING TO TRACK BEING TESTED!!
csv_path = os.path.join(current_dir, f"{track_name}_track_data.csv")
plot_track_3d(csv_path, track_name)
